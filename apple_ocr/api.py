...
class AppleOCR:
    ...
    def extract_text_from_images(
        ...
        try:
            ...
            for res in ocr_client.collect_results(expected_pages=len(indexed_images)):
                # 找回对应图片名
                img_name = next((p.name for (i, p, _, _) in indexed_images if i == res.page_index), None)
                page_data = {
    ...
    def extract_text_from_image_dir(
        ...
        allow = ["png", "jpg", "jpeg", "tiff", "bmp"] if not exts else [e.lower() for e in exts]
        files = sorted([p for p in d.iterdir() if p.is_file() and p.suffix.lower().lstrip('.') in allow])
        return self.extract_text_from_images(files)
